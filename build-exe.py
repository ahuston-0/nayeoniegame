...
try:
    ...
    # py2exe is required to build exe, but is windows only
    import py2exe  # noqa: F401 # ty: ignore[unresolved-import]
    import pygame
except ImportError as message:
    raise SystemExit("Unable to load module.") from message
    ...
    def find_data_files(self, srcdir, *wildcards, **kw):
        ...
        if recursive:
            for dirname, _, files in os.walk(srcdir):
                walk_helper((file_list, wildcards), dirname, files)
        else:
            walk_helper(
                (file_list, wildcards),
                srcdir,
